...
# guard against industries spawning spontaneously; unlocks are fundable-only in this fork
for industry in registered_industries:
    for economy in registered_economies:
        if industry.economy_variations[economy.id].enabled:
            prob_in_game = industry.get_property("prob_in_game", economy)
            if industry.id != "general_store":
                assert prob_in_game == "0", (
                    "%s has prob_in_game %s; fork policy is 0 for all industries except general_store"
                    % (industry.id, prob_in_game)
                )
